...
const IDENTIFIER: &str = "prepared-statements";
const SLOTS: usize = 1;
const WAIT_TIMEOUT: Duration = Duration::from_secs(10);

/// The way each driver turns server-side prepared statements off, which the
/// refusal must spell out so that whoever hits it can recover on their own.
...
#[tokio::test]
async fn a_driver_that_prepares_a_statement_is_told_how_to_turn_it_off() {
    ...
    let error = client
        .query(&statement, &[])
        .await
        .expect_err("a statement prepared before the boundary must not bind after it");
    let refusal = error.as_db_error().expect("a database error");
    assert_eq!(*refusal.code(), SqlState::INVALID_SQL_STATEMENT_NAME);
    assert!(
        refusal.message().contains("prepared statement"),
        "{}",
        refusal.message()
    );
    ...
    for disable in DISABLE_HINTS {
        assert!(hint.contains(disable), "{hint}");
    }
